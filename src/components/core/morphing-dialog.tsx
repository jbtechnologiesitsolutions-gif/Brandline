...
export function MorphingDialogTrigger({
  ...
  const { setIsOpen, isOpen, uniqueId, triggerRef: defaultTriggerRef } =
    useMorphingDialog();
...
export function MorphingDialogClose({
  ...
  return (
    <motion.button
      ...
      className={cn('absolute top-4 right-4 z-10 rounded-full p-2 text-foreground/70 transition-colors hover:bg-muted hover:text-foreground', className)}
      initial='initial'
      animate='animate'
      exit='exit'
      variants={variants}
